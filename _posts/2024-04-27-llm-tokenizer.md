...
![BPE token learner](/assets/img/llm-tokenizer/bpe.png)
...
该分词方法基于 Unigram 语言模型，认为当前词的出现不依赖于前面的词，因此子词序列 $\mathbf{x} = (x_1, \cdots, x_M)$ 的概率将表示为：

$$
P(\mathbf{x}) = \prod_{i=1}^{M}p(x_i)
$$

对于输入文本 $X$，对于 $X$ 的所有可能分割 $S(X)$，可用 Viterbi 算法求解**最大概率路径**得到最优分割：

$$
\mathbf{x}^* = \underset{\mathbf{x} \in S(X)}\arg\max P(\mathbf{x})
$$
...
![BPE Dropout Process](/assets/img/llm-tokenizer/bpe-dropout.png)
...
| `o200k_base`  | GPT4o                   | 199,998 及 2 个特殊符号 |
